...
// FIX: Using UTC methods to keep dates stable globally
function fmtDate(dateStr) {
  ...
  const fMonth = MONTHS_SHORT[friday.getUTCMonth()];
  const sMonth = MONTHS_SHORT[sunday.getUTCMonth()];

  if (friday.getUTCMonth() !== sunday.getUTCMonth()) {
    return `${fMonth} ${fDay} – ${sMonth} ${sDay}`;
  }
  return `${sMonth} ${fDay} – ${sDay}`;
}
...
const STATIC_WINNERS = { '1': 'G. Russell', '2': 'K. Antonelli', '3': 'K. Antonelli' };
      ...
      <div className="cal-strip-wrap">
        ...
        <div className="cal-strip" ref={stripRef}>
          {displayRaces.map((race, i) => {
            ...
            // FIX: Using the fmtDate helper function instead of manual calculation
            const dateStr = fmtDate(race.date);
